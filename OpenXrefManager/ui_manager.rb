...
module OpenXrefManager
  ...
  def self.get_xref_data_as_json
    ...
    data = xref_definitions.map do |definition|
      ...
      if !path || !File.exist?(path)
        ...
        status = if lock_owner_name && !lock_owner_name.empty?
                   "Read-Only Checkout (Locked by #{lock_owner_name})"
                 else
                   "Read-Only Checkout"
                 end
        status_key = "readonly_checkout"
        # If modified in readonly checkout, show as update available
        update_available = true if readonly_modified
      elsif is_locked
      ...
      definition.get_attribute(XREF_DICT_NAME, XREF_LAST_PUBLISHER_MODEL_KEY)
  ...
  def self.show_manager_dialog
    if @dialog&.visible?
      ...
      @dialog.add_action_callback("refresh_data") do |_action_context|
        refresh_dialog_data
      end
      ...
      @dialog.add_action_callback("select_component_clicked") do |_action_context, component_name|
        select_component_instances(component_name)
      end

      @dialog.add_action_callback("check_out_clicked") do |_action_context, component_name|
        check_out_xref(component_name)
      end

      @dialog.add_action_callback("check_out_readonly_clicked") do |_action_context, component_name|
        check_out_xref_readonly(component_name)
      end

      @dialog.add_action_callback("save_and_check_in_clicked") do |_action_context, component_name|
        save_and_check_in_xref(component_name)
      end
      ...
      @dialog.add_action_callback("force_unlock_clicked") do |_action_context, component_name|
        force_unlock_xref(component_name)
      end

      @dialog.add_action_callback("reload_single_clicked") do |_action_context, component_name, is_update|
        ...
        reload_single_xref(component_name)
      end

      @dialog.add_action_callback("unload_single_clicked") do |_action_context, component_name|
        unload_single_xref(component_name)
      end

      @dialog.add_action_callback("relink_clicked") do |_action_context, component_name|
        relink_xref(component_name)
      end

      @dialog.add_action_callback("unlink_single_clicked") do |_action_context, component_name|
        unlink_single_xref(component_name)
      end

      @dialog.add_action_callback("enable_travel_through_clicked") do |_action_context, component_name|
        enable_travel_through(component_name)
      end

      @dialog.add_action_callback("disable_travel_through_clicked") do |_action_context, component_name|
        disable_travel_through(component_name)
      end

      @dialog.add_action_callback("cancel_readonly_checkout_clicked") do |_action_context, component_name|
        cancel_readonly_checkout(component_name)
      end

      @dialog.add_action_callback("publish_all_clicked") do |_action_context, keep_locked_str|
        ...
        save_and_check_in_all_my_xrefs(keep_locked: keep_locked)
      end

      @dialog.add_action_callback("get_keep_locked_preference") do |_action_context|
      ...
      @dialog.add_action_callback("get_monitoring_state") do |_action_context|
        # Return the current monitoring paused state
        is_paused = monitoring_paused?
        @dialog.execute_script("setMonitoringButtonState(#{is_paused});")
      end

      @dialog.add_action_callback("purge_unused_clicked") do |_action_context|
        purge_unused_xrefs
      end

      @dialog.add_action_callback("toggle_path_type_clicked") do |_action_context, component_name|
        toggle_path_type(component_name)
      end

      @dialog.add_action_callback("set_user_clicked") do |_action_context|
        set_user_name
      end

      @dialog.add_action_callback("settings_clicked") do |_action_context|
        show_settings_dialog
      end

      @dialog.add_action_callback("pause_monitoring_clicked") do |_action_context|
        pause_monitoring
      end

      @dialog.add_action_callback("resume_monitoring_clicked") do |_action_context|
        resume_monitoring
      end

      @dialog.add_action_callback("reload_all_clicked") do |_action_context|
        reload_all_xrefs
      end

      @dialog.add_action_callback("import_origin_clicked") do |_action_context|
        import_as_xref_at_origin
      end

      @dialog.add_action_callback("import_place_clicked") do |_action_context|
        import_as_xref
      end

      @dialog.add_action_callback("create_xref_clicked") do |_action_context|
        create_xref_from_component
      end

      # New callback for JS confirmation dialogs
      @dialog.add_action_callback("request_confirmation") do |_action_context, message, callback_name, param|
  ...
  def self.refresh_dialog_data
    ...
    json_data = get_xref_data_as_json
  ...
  def self.ask_publish_before_operation?(component_name, operation_name)
    ...
    if xref_has_uncommitted_changes?(definition)
  ...
  def self.confirm_close_with_uncommitted_changes?(action_text = "continue")
    ...
    if result == IDYES
      show_manager_dialog
      false # Abort the original action (e.g. closing model) so user can publish
    else
      result == IDNO # Cancel
    end
  end
