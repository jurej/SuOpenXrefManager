...
module OpenXrefManager
  ...
  class OpenXrefEntitiesObserver < Sketchup::EntitiesObserver
    def onElementModified(entities, entity)
      ...
      if is_locked_by_file
        lock_owner_name, lock_owner_guid, lock_model_name, = lock_content.split("|")
  ...
  class OpenXrefModelObserver < Sketchup::ModelObserver
    ...
    def onActivePathChanged(model)
      ...
      if travel_through_enabled_for_parent
        ...
        model.active_path.each do |path_instance|
          ...
          parent_def = path_instance.definition
          unless OpenXrefManager.is_xref?(parent_def) && parent_def != definition && OpenXrefManager.is_travel_through_enabled?(parent_def)
            next
          end
      ...
      if (is_locked_by_other || is_mine_elsewhere) && OpenXrefManager.is_travel_through_enabled?(definition)
        ...
        unless OpenXrefManager.instance_variable_get(:@travel_through_active)
          OpenXrefManager.instance_variable_set(:@travel_through_active,
                                                {})
        end
      ...
      if is_locked_by_other
        UI.start_timer(0.1, false) do
          ...
          if result == IDYES
            ...
            definition.set_attribute(OpenXrefManager::XREF_DICT_NAME, OpenXrefManager::XREF_EDITED_WITHOUT_LOCK_KEY,
                                     false)
      ...
      return unless is_unlocked || is_mine_elsewhere || travel_through_enabled_for_parent
    ...
    def onBeforeComponentSaveAs(instance)
      ...
      true # Allow the save operation.

      # Prevent the save operation.
    end

    # This method is called AFTER the 'Save As' operation completes.
    def onAfterComponentSaveAs(_instance)
      return unless @definition_to_relink&.valid?
